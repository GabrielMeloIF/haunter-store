...
export default function Cards() {
  ...
  const categoriaConsoles = categorias.find(
    (categoria) => categoria.nome_categoria === "Consoles",
  );

  const produtosConsoles = produtos.filter(
    (produto) => produto.categoriaId === categoriaConsoles?.id_categoria,
  );
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center gap-10 py-8">
        <h2 className="text-white font-bold text-3xl px-6 mb-6">Consoles</h2>

        <div className="grid grid-cols-3 gap-8 px-6">
          {produtosConsoles.map((produto) => (
            <CardItem key={produto.id} produto={produto} />
          ))}
        </div>

        {produtosConsoles.length === 0 && (
          <p className="text-white text-xl">Nenhum console encontrado.</p>
        )}
      </div>
