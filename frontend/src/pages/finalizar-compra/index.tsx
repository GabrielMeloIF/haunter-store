...
import { todosProdutos, Produto } from "../../produtos/index";
...
function calcularParcela(preco: string, parcelas: number): string {
  const valor = Number(preco.replace("R$", "").replace(".", "").replace(",", ".").trim());
  if (parcelas === 1) return `R$ ${valor.toFixed(2).replace(".", ",")}`;
  const valorComJuros = valor * Math.pow(1 + JUROS_POR_PARCELA, parcelas);
  return `R$ ${(valorComJuros / parcelas).toFixed(2).replace(".", ",")}`;
}

function calcularTotal(preco: string, parcelas: number): string {
  const valor = Number(preco.replace("R$", "").replace(".", "").replace(",", ".").trim());
  if (parcelas === 1) return `R$ ${valor.toFixed(2).replace(".", ",")}`;
  const total = valor * Math.pow(1 + JUROS_POR_PARCELA, parcelas);
